...
Respond with ONLY the fixed JSON.`;
                    ...
                    const repairedResponse = await callAnthropic(
                        repairPrompt,
                        0,
                    );
                    const repairedValidation = validateAndRepair(
                        ParsedTripSchema,
                        repairedResponse,
                    );
            ...
            case "generate_day": {
                ...
                const prompt = buildGenerateDayPrompt(
                    day,
                    settings,
                    itineraryContext,
                );
            ...
            case "generate_all_days": {
                ...
                for (const chunk of chunks) {
                    const prompt = buildGenerateAllDaysPrompt(
                        chunk,
                        settings,
                        itineraryContext,
                    );
